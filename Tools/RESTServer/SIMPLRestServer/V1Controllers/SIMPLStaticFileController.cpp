...
void SIMPLStaticFileController::service(HttpRequest& request, HttpResponse& response)
{
    ...
    else
    {
        ...
        if (path.contains("/.."))
        {
            qWarning("SIMPLStaticFileController: detected forbidden characters in path %s",path.data());
        ...
        qDebug("SIMPLStaticFileController: Open file %s",qPrintable(file.fileName()));
